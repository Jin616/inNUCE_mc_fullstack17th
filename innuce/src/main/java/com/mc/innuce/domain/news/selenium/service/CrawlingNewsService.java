...
public class CrawlingNewsService {
	...
	final private int limitCrawlingSearchSortInterest = 5;
	...
	public List<NewsDTO> getSearchNewsDefault(String keyword) {
		WebDriver driver = WebDriverPool.getWebDriver();
		String url = naverSearchDefaultUrl + keyword;
		driver.get(url);
		...
		while(true) {
			...
			try {
				...
				filterList.addAll(bxList.stream().filter(e -> 
				e.findElements(By.className("dsc_thumb")).size() == 1
				...
				for(WebElement e : filterList) {
					String href = e.findElement(By.className("info_group")).findElements(By.tagName("a")).get(1).getAttribute("href");
					
					if(dbHash.add(conv.get13NewsKey(href))) {
						String thumburl = e.findElement(By.className("dsc_thumb")).findElement(By.tagName("img")).getAttribute("src");
						newsVOList.add(new NewsTemVO(href, thumburl));
					}
					if(newsVOList.size() == returnDefaultSize)
						break;
				}
				...
				if(newsVOList.size() < returnDefaultSize && retryCount < maxRetry) {
		...
		for(NewsTemVO vo : newsVOList)
			dtoList.add(parseNewsToNewsDTO(driver, vo.getUrl(), vo.getThumburl()));
		
		newsService.insertNewsList(dtoList);
...
//		}
	...
	private List<String> searchNews(WebDriver driver, String keyword, LocalDate date) {
		...
		resultList.addAll(list.stream().filter(e -> 
			e.findElements(By.className("dsc_thumb")).size() == 1
			&& e.findElement(By.className("info_group")).findElements(By.tagName("a")).size() == 2).collect(Collectors.toList()));
		...
		for(WebElement e : list) {
			if(e.findElements(By.className("dsc_thumb")).size() == 1
			&& e.findElement(By.className("info_group")).findElements(By.tagName("a")).size() == 2)
			resultList.add(e);
		}
		after = System.currentTimeMillis();
	...
	public int getLimitPagePerCategory(String category) {
		...
		while(true) {
			try {
				...
				limitPage = Integer.parseInt(driver.findElement(By.id("paging")).findElement(By.tagName("strong")).getText());
				break;
			} catch (Exception e) {
				driver.get(url);
				sleep(1000);
...
//						System.out.printf("insert %d complete %d\n", resultList.size(), newsService.insertNewsList(resultList));
	...
	public NewsDTO parseNewsToNewsDTO(WebDriver newsdriver, String href, String thumbnailuri) {
		...
		if(news.getSumm_content() == null) news.setSumm_content("원문참조");
