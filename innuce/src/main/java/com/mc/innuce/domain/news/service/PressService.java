...
import com.mc.innuce.domain.news.dao.DBUpdateTimeDAO;
...
public class PressService {
	...
	PressDAO pressDAO;
	@Autowired
	DBUpdateTimeDAO dbDAO;
	
	/**
	 * 서버 실행 시 혹은 필요할 때마다 언론사 정보를 새로 받아와서 이 메서드를 실행.
	 * DB에 이미 언론사 키가 존재하고, overriding한 equals 메서드를 통해 다른 객체임이 확인 되면 updateList에,
	 * DB에 언론사 키가 없다면 insertList에 pressList의 dto 값들을 넣어준다.
	 * DB에 각각의 기능을 실행 후 결과값 출력 후 종료
	 */
	...
	public void updatePressTable(List<PressDTO> pressList) {
		...
		if(insertList.size() != 0) insertRow += pressDAO.insertListPress(insertList);
	...
	public Calendar getPressUpdateTime() {
		Timestamp time = dbDAO.selectPressUpdateTime();
	...
	public void updatePressUpdateTime() {
		dbDAO.updatePressUpdateTime();
	}
