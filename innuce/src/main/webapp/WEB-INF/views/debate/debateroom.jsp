	...
	$(document).ready(
			function() {
				...
				stomp.connect({}, function() {
					...
					stomp.subscribe('/sub/debate/' + roomId, function(
							debateMessage) {
					...
					// 채팅방 인원 정보 메시지 구독
					stomp.subscribe('/sub/debate/participants/' + roomId,
					...
					// 채팅방 입장시 메시지 전송
					// 실시간 참여 인원 + 1
					stomp.send('/pub/debate/connected/' + roomId, {}, JSON
							.stringify({
								message : debateuser
							}));
				});
				...
				$(window).on(
						...
						function() {
							...
							stomp.send('/pub/debate/disconnected/' + roomId,
									{}, JSON.stringify({
										message : debateuser
									}));
							// stomp를 disconnect
				...
				$('#leavebtn').on(
						...
						function() {
							...
							stomp.send('/pub/debate/leaveroom/' + roomId, {},
									JSON.stringify({
										message : debateuser
									}));
							// 이전 페이지 url 호출
		...
		$.ajax({
			...
			success : function(response) {
				...
				for (let i = 0; i < response.length; i++) {
					...
					preMessage(createMessage(jsonobj),
							$('.debate_room_opinion_list'));
				}
			},
			error : function(request, e) {
				alert("코드 : " + request.status + " 메시지 : "
						+ request.responseText + " 오류" + e);
			}
		});
